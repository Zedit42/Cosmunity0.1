...
const fetchData = async () => {
    const apiUrl = `${process.env.NEXT_PUBLIC_API_URL}/group`;
...
const ChatBox = ({ walletAddress, tokenAmount }) => {
  ...
  return (
    <div className='flex flex-col'>
        <div className=' flex  justify-between border-b-[5px] border-black gap-5 '>
            <img
                src='/logo.png'
                width={64}
                height={64}
            ...
            <p className=' text-sm w-1/6'> AKT {tokenAmount}</p>
        </div>
        <div>
        ...
        <div>
            <div className=' overflow-auto flex flex-col max-h-[75vh]'>
            {groups.map((group) => (
                <Link href={`/chat/${group.id}`} key={group.id} className=" grouppixelbg bg-white flex flex-row  overflow-hidden min-h-[60px]">
                    <img className="w-full object-cover object-center" src={`picture${(group.id % 3) + 1}.svg`} alt="Group Picture" />
                    <div className="flex flex-col max-w-[80%]">
                        <h2 className="text-lg font-semibold">{group.grupName}</h2>
